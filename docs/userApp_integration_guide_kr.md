...
try await GoPassSDK.shared.initialize(apiKey: apiKey)
```
...
| 타입 | 값 | 권장 처리 |
|----|------|-----------|
| BioDataFrameStatus | continueCapture | 카메라 캡처 유지, 다음 프레임 전달 |
...
|-----------|---------------|------|
| GP001 | `.remoteTransactionFailed` | 인증 처리에 실패했습니다. |
| GP002 | `.locationServiceDisabled` | 위치 서비스 권한이 필요합니다. |
| GP003 | `.bluetoothDisabled` | 블루투스를 활성화해주세요. |
| GP004 | `.noServiceArea` | 서비스 가능 위치가 아닙니다. |
| GP005 | `.networkDisconnected` | 네트워크 연결을 확인해주세요. |
| GP006 | `.sessionFailed` | 세션 수립에 실패했습니다. |

#### 5.1.2 GP1xx — `initialize(apiKey:)`

| 에러 코드 | SDKError case | 설명 |
|-----------|---------------|------|
| GP101 | `.invalidParameters` | 요청 파라미터가 올바르지 않습니다. |
| GP102 | `.initNetworkDisconnected` | 네트워크 연결을 확인해주세요. |
| GP103 | `.initializationFailed` | 초기화를 실패했습니다. |
| GP104 | `.updateSDK` | SDK 버전이 최신이 아닙니다. |
| GP105 | `.internalNetworkError` | 내부 네트워크 통신에 실패하였습니다. |

#### 5.1.3 GP2xx — `registerBioData()`

| 에러 코드 | SDKError case | 설명 |
|-----------|---------------|------|
| GP201 | `.invalidImageData` | 이미지 데이터가 누락되었거나 형식이 올바르지 않습니다. |
| GP202 | `.registerBioDataFailed` | 생체 데이터 등록에 실패했습니다. |
| GP203 | `.bioDataisEmpty` | 생체 데이터가 존재하지 않습니다. |

#### 5.1.4 GP3xx — `removeBioData()`

| 에러 코드 | SDKError case | 설명 |
|-----------|---------------|------|
| GP301 | `.removeBioDataFailed` | 생체 데이터 삭제에 실패했습니다. |
...
// function
do {
    let userId = try await GoPass.shared.initialize(apiKey: key)
} catch let error as SDKError {
...
NotificationCenter.default.addObserver(
    ...
    print("[\(error.code)] \(error.message)")   // "[GP003(IN-002)] 블루투스를 활성화해주세요."
}
```
