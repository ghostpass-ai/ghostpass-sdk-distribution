...
> **버전** 1.0 · **지원 플랫폼** iOS 15.0+ · **배포 형식** XCFramework  
> **최종 업데이트** 2025년
...
1. [개요](#1-개요)
2. [시작하기 (Quick Start)](#2-시작하기)
3. [SDK 초기화](#3-sdk-초기화)
4. [얼굴 등록 (Enrollment)](#4-얼굴-등록)
5. [얼굴 인증 (Authentication)](#5-얼굴-인증)
6. [에러 처리](#6-에러-처리)
7. [고급 설정](#7-고급-설정)
8. [FAQ](#8-faq)
...
```
initialize()  →  registerFace()  →  결과 콜백 처리
```
...
| 기능 | 설명 |
|------|------|
| 안면 등록 | 카메라 이미지로부터 얼굴 특징 벡터(FVector) 추출 및 Keychain 저장 |
| Liveness Detection | RGB 기반 안티 스푸핑으로 사진·영상 위조 방지 |
| 안면 인증 | 등록된 특징 벡터와 실시간 얼굴 비교 |
| 보안 저장 | 모든 민감 데이터는 iOS Keychain에만 저장 |
| 로그 / FRR 수집 | 인증 시도 로그와 FRR(False Rejection Rate) 데이터를 CSV로 추출 |
...
② SDK 초기화 (앱 실행 시 1회)
   gpInit(apiKey:) 호출 → SDK 내부 보안 채널 자동 수립 → onSDKReady() 콜백

③ 얼굴 등록 (사용자당 1회)
   gpSaveLandmark(image:) 호출 → Liveness 검사 + 특징 추출 + 안전 저장

④ 자동 인증 (키오스크 근접 시 자동 동작)
   키오스크 비콘 감지 → 인증 세션 수립 → 기기 내 얼굴 매칭
   → delegate 콜백으로 인증 결과 수신
```
...
| 단계 | 호출 | SDK 자동 처리 |
|------|------|---------------|
| 초기화 | `gpInit(apiKey:)` 1회 | 기기 검증 · 보안 채널 수립 · 내부 키 관리 |
| 얼굴 등록 | `gpSaveLandmark(image:)` | Liveness 검사 · 특징 추출 · 안전한 저장 |
| 결과 수신 | `delegate` 콜백 구현 | 비콘 스캔 · 세션 관리 · 암호화 · 매칭 전부 |
...
### 2.1 설치 — Private GitHub Repository (Swift Package Manager)
...
```
https://github.com/ghostpass-ai/iOS_UC_SDK
```
...
카메라 접근을 위해 아래 키를 `Info.plist`에 반드시 추가해야 합니다.

```xml
<key>NSCameraUsageDescription</key>
<string>안면 인식 인증을 위해 카메라 접근이 필요합니다.</string>
```

> ⚠️ **주의**: 권한 설명 문자열을 누락하거나 너무 모호하게 작성하면 App Store 심사에서 거부될 수 있습니다. 카메라를 사용하는 정확한 이유를 명시하세요.

### 2.4 Firebase 설정 (푸시 알림 사용 시)

샘플 앱은 Firebase를 통한 푸시 알림을 지원합니다. 푸시 기능이 필요한 경우:

1. Firebase 콘솔에서 iOS 앱을 등록하고 `GoogleService-Info.plist`를 다운로드합니다.
2. 파일을 프로젝트 루트에 추가합니다.
3. `AppDelegate`에서 `FirebaseApp.configure()`를 호출합니다.

> 💡 **팁**: 안면 인증 기능만 필요한 경우 Firebase는 선택 사항입니다.
...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(
        ...
        // GhostPass SDK 초기화
        GPSDK.shared.gpInit { result in
            switch result {
            case .success(let model):
                print("✅ SDK 초기화 성공")
                if let model = model {
                    print("유저 얼굴 OID: \(model.userFacialOid)")
                }
            case .failure(let error):
                print("❌ SDK 초기화 실패: \(error.errorDescription ?? "")")
            }
        }
...
> ⚠️ **주의**: `gpInit` 완료 콜백은 **메인 스레드가 아닐 수 있습니다**. UI 업데이트가 필요한 경우 반드시 `DispatchQueue.main.async { }` 블록 안에서 처리하세요.

---

## 4. 얼굴 등록

얼굴 등록은 사용자의 안면 특징 벡터를 기기의 Keychain에 저장하는 과정입니다.
...
카메라 이미지 캡처
       ↓
gpSaveLandmark(image:) 호출
       ↓
  ┌────┴─────┐
  ↓          ↓
성공        실패
Keychain    에러 콜백
저장 완료    (GPFaceDetectionError)
```
...
### 6.1 GPFaceDetectionError — 얼굴 감지 관련
...
SDK는 내부적으로 `log(_:)` 함수를 통해 표준화된 디버그 로그를 제공합니다.

```swift
// 에러 발생 시 자동으로 아래 형식의 로그가 출력됩니다:
// [GPBasicError] op=compareLandmark
//  fn=gpCompareLandMark(hostLocationOid:secureData:_:)
//  GoPassSDK.swift:56
//  reason=저장된 랜드마크가 없습니다.
//  recovery=랜드마크를 먼저 등록하세요.
```

### 7.2 CSV 로그 / FRR 데이터 수집

테스트 및 품질 분석을 위한 로그 수집 기능을 제공합니다.

```swift
// 로그 데이터 추가
let logEntry = LogFormat(
    attemptType: "Authentication",
    outcome: "SUCCESS",
    enrollmentLandmark: nil,
    compareLandmark: [],
    hostlocationOid: 12345,
    livenessConfidence: 0.95,
    compareResult: 0.28
)
GPSDK.shared.gpAddLogData([logEntry])

// CSV 파일로 내보내기 (공유 시트 표시)
GPSDK.shared.gpDownloadLogCSV(fileName: "auth_log.csv") { success in
    print(success ? "✅ 내보내기 완료" : "❌ 내보내기 실패")
}

// FRR 데이터 내보내기
GPSDK.shared.gpDownloadFRRCSV(fileName: "frr_data.csv")

// 데이터 초기화
GPSDK.shared.gpClearLogData()
GPSDK.shared.gpClearFRRData()
```
...
```swift
public class GPSDK {
    public static let shared: GPSDK
}
```
